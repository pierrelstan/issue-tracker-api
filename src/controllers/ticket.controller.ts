...
// import { pusher, pusherChannel } from '../../index';
 ...
 * @access   Private
 */
export const updateTicketById = async (req: any, res: Response) => {
    try {
        const { id } = req.params;
        const {
            title,
...
export const createTicketComment = async (
    req: AuthorizedRequest<CommentType>,
    res: Response
) => {
    try {
        const { id } = req.params;
        ...
        const ticket: any = await Ticket.findById(id).populate(
            "project",
            "author"
        );
        ...
        if (!ticket)
            return res
                .status(404)
                .json({ message: "Comment's ticket not found" });
...
export const getTicketComment = async (req: any, res: Response) => {
    try {
